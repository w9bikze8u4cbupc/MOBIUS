...
async function testSizeProximity() {
  ...
  result.images.forEach((img, i) => {
    ...
    if (i < 3) {
      console.log(`\n🖼️  Image ${i + 1}: ${img.url}`);
      console.log(`   Size: ${img.scores?.sizeScore?.toFixed(4) || 'N/A'} (${img.width || img.w}x${img.height || img.h})`);
      console.log(`   Proximity: ${img.scores?.proximityScore?.toFixed(4) || 'N/A'} (distance: ${img.sectionDistance})`);
  ...
  const { harvestAllImages: harvestAllImagesWithProbing } = await import('./scripts/harvest-images.js');
