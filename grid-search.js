...
const grids = {
  wSize:      [0.20,0.25,0.30,0.35,0.40],
  wProx:      [0.20,0.25,0.30,0.35,0.40],
  wProv:      [0.10,0.15,0.20,0.25,0.30],
  wFocus:     [0.15,0.20,0.25,0.30],
  wUnique:    [0.05,0.10,0.15,0.20],
  hiThresh:   [0.72,0.75,0.78,0.80],
  medThresh:  [0.50,0.55,0.60]
};

// Scoring function
function score(r, w) {
  const s = w.wSize*r.size + w.wProx*r.proximity + w.wProv*r.providerWeight + w.wFocus*r.focus + w.wUnique*r.uniqueness;
...
for (const med of grids.medThresh) {
  ...
  let tpH=0, fpH=0, tpHM=0, fpHM=0;
  
  for (const r of rows) {
    const s = score(r, w);
    const band = s >= hi ? 'H' : s >= med ? 'M' : 'L';
    if (band === 'H') { r.isPositive ? tpH++ : fpH++; }
    if (band === 'H' || band === 'M') { r.isPositive ? tpHM++ : fpHM++; }
  }
  ...
  if (!best || cand.objective > best.objective) {
    best = cand;
    console.error(`New best: objective=${cand.objective}, precisionH=${cand.precisionH}, recallHM=${cand.recallHM}`);
  }
}
