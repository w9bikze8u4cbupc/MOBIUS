...
function improvedMatchComponentsToImages(components, allImages) {
  ...
  const synonymGroups = {
    cards: ['card', 'cards', 'deck', 'playing card', 'game card', 'action card', 'event card', 'character card', 'spell card', 'treasure card'],
    dice: ['die', 'dice', 'd6', 'd4', 'd8', 'd10', 'd12', 'd20', 'six-sided', 'four-sided', 'custom die'],
    tokens: ['token', 'tokens', 'marker', 'markers', 'chip', 'chips', 'counter', 'counters', 'disc', 'discs'],
    meeples: ['meeple', 'meeples', 'figure', 'figures', 'pawn', 'pawns', 'worker', 'workers', 'piece', 'pieces'],
    boards: ['board', 'gameboard', 'game board', 'main board', 'player board', 'side board'],
    tiles: ['tile', 'tiles', 'hex', 'hexes', 'hexagon', 'square', 'squares'],
    cubes: ['cube', 'cubes', 'wooden cube', 'resource cube', 'block', 'blocks'],
    money: ['money', 'coin', 'coins', 'currency', 'cash', 'gold', 'silver', 'bronze'],
    miniatures: ['miniature', 'miniatures', 'mini', 'minis', 'figurine', 'figurines', 'standee', 'standees']
  };
  ...
  function calculateImageScore(component, image) {
    ...
    if (imageName.includes(componentName) || componentName.includes(imageName)) {
      score += 0.9;
    ...
    if (componentCategory) {
      ...
      const imageWordsMatch = categoryWords.some(word => imageName.includes(word) || imagePath.includes(word));
      if (imageWordsMatch) {
        score += 0.7;
    ...
    if (imagePath.includes('component') || imagePath.includes('token') || imagePath.includes('card')) {
      score += 0.2;
  ...
  return components.map(component => {
    ...
    const alternatives = scoredImages.slice(1, 4).filter(match => match.score > 0.3);
    ...
    return {
      ...
      matchReason: bestMatch ? 
        ...
        "No good match found",
      alternativeMatches: alternatives.map(alt => ({ image: alt.image, score: alt.score })),
...
function splitMarkdownSections(markdown) {
  ...
  while ((match = regex.exec(markdown)) !== null) {
    if (match.index > lastIndex) sections.push(markdown.slice(lastIndex, match.index).trim());
  ...
  if (lastIndex < markdown.length) sections.push(markdown.slice(lastIndex).trim());
...
function App() {
  ...
  const [bggUrl, setBggUrl] = useState(() => localStorage.getItem("bggUrl") || "");
  ...
  function openWebSearchBatch() {
    ...
    const emptyIndices = matchedComponents.reduce((arr, c, i) => {
      const hasAny = Boolean(c.suggestedImage) || (c.selectedImages && c.selectedImages.length > 0);
  ...
  function toggleWebSelect(img) {
    ...
    setWebSelected(prev => (prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]));
  }

  async function runWebSearch() {
    if (!gameName.trim() && !(metadata?.bgg_id || bggMetadata?.bgg_id)) {
    ...
    try {
      ...
      const res = await axios.post(`${API_BASE}/api/search-images`, body, {
        headers: { "Content-Type": "application/json" }
      });
  ...
  function togglePickerImage(img) {  
    const key = imgKey(img);  
    setPickerTemp(prev => (prev.includes(key) ? prev.filter(k => k !== key) : [...prev, key]));  
  }  
  ...
  function applyPickerSelection() {  
    ...
    if (!compImageOverrides[pickerOpenIdx] && pickerTemp.length > 0) {  
      setCompImageOverrides(prev => ({ ...prev, [pickerOpenIdx]: pickerTemp[0] }));  
    }  
    closeImagePicker();  
  }  
  ...
  const matchedComponents = useMemo(() => {
    ...
    const processed = base.map((c, idx) => {
      ...
      if (overrideKey) {
        ...
        return img
          ? { ...c, suggestedImage: img, selectedImages: selectedImgs, matchConfidence: 1, matchReason: "Manual assignment" }
  ...
  const handleDropOnComponent = compIndex => e => { // uses compImageMulti state declared above
    e.preventDefault();
  ...
  const toggleImageForComponent = (compIndex, imageKey) => {
    setCompImageMulti(prev => {
      ...
      const nextArr = exists ? prevArr.filter(k => k !== imageKey) : [...prevArr, imageKey];
  ...
  const applyImageSelection = () => {
    setAllImages(prev => prev.filter(img => !excludedImageIds.has(img.id || img.path)));
    setExtractedImages(prev => prev.filter(img => !excludedImageIds.has(img.id || img.path)));
  ...
  useEffect(() => {
    if (bggMetadata) {
      setMetadata(prev => ({
        ...
        publisher: Array.isArray(bggMetadata.publisher) ? bggMetadata.publisher.join(", ") : bggMetadata.publisher || "",
        player_count: bggMetadata.player_count || "",
        play_time: bggMetadata.play_time || "",
        min_age: bggMetadata.min_age || "",
        theme: Array.isArray(bggMetadata.theme) ? bggMetadata.theme.join(", ") : bggMetadata.theme || "",
        edition: "",
        bgg_id: bggMetadata.bgg_id || "",
        bgg_rank: bggMetadata.bgg_rank || "",
        mechanics: Array.isArray(bggMetadata.mechanics) ? bggMetadata.mechanics.join(", ") : bggMetadata.mechanics || "",
        artists: Array.isArray(bggMetadata.artists) ? bggMetadata.artists.join(", ") : bggMetadata.artists || "",
        designers: Array.isArray(bggMetadata.designers) ? bggMetadata.designers.join(", ") : bggMetadata.designers || "",
  ...
  const handleGenerateStoryboard = async () => {
    try {
      ...
      const res = await axios.post(`${API_BASE}/api/generate-storyboard`, payload, {
        headers: { "Content-Type": "application/json" }
      });
      setStoryboard(res.data.storyboard || null);
    } catch (err) {
      console.error("Storyboard generation failed:", err);
      setError("Failed to generate storyboard: " + (err.response?.data?.error || err.message));
    } finally {
  ...
  const handleExtractBGG = async () => {
    try {
      ...
      if (response.data && response.data.success && response.data.metadata) {
        setBggMetadata(response.data.metadata);
        localStorage.setItem("bggMetadata", JSON.stringify(response.data.metadata));
      } else {
  ...
  const handleStartExtraction = async () => {
    try {
      ...
      if (invalid.length > 0) {
        setError(`Some entries are not valid URLs and were ignored: ${invalid.join(", ")}`);
      }
      ...
      if (hasExtra) {
        const response = await axios.post(
          `${API_BASE}/api/extract-extra-images`,
          { extraImageUrls: urls.join(",") },
          { headers: { "Content-Type": "application/json" } }
        );
        if (response.data && response.data.extraImages) {
          setExtractedImages(prev => [...prev, ...response.data.extraImages]);
          setAllImages(prev => [...prev, ...response.data.extraImages]);
        }
  ...
  const handleFetchBGGImages = async () => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE}/api/fetch-bgg-images`,
        { gameName },
        { headers: { "Content-Type": "application/json" } }
      );
      if (response.data.success) {
        const bggImgs = Array.isArray(response.data.images) ? response.data.images : [];
  ...
  const handlePdfImagesSelected = (selectedImages, metadata) => {
    console.log('PDF images selected:', selectedImages, 'Job metadata:', metadata);
    ...
    setAllImages(prev => {
      ...
      const newImages = selectedImages.filter(img => !existing.has(img.url || img.path));
      return [...prev, ...newImages];
    });
    
    // Show success message
    const count = selectedImages.length;
    setError(`✓ Successfully added ${count} image${count !== 1 ? 's' : ''} from PDF extraction (Job: ${metadata?.jobId})`);
  ...
  const handleFile = async file => {
    ...
    try {
      ...
      const pdfPath = uploadRes.data.pdfPath;
      const imagesFromPDF = uploadRes.data.images || [];
      setPdfPath(pdfPath);
      ...
      const compRes = await axios.post(
        `${API_BASE}/api/extract-components`,
        { pdfPath },
        { headers: { "Content-Type": "application/json" } }
      );
      const extractedComponents = compRes.data.components || [];
      setComponents(extractedComponents);
      setExtractionMethod(compRes.data.extractionMethod || "");
      setExtractionStats(compRes.data.extractionStats || null);
      setExtractionMessage(compRes.data.message || "");
      setTimeout(() => {
      ...
      if (extractedComponents.length > 0) {
        localStorage.setItem(`components_${pdfPath}`, JSON.stringify(extractedComponents));
      }
  ...
  const handleExtractExtraImages = async () => {
    ...
    try {
      ...
      if (urls.length === 0) {
        setError(prev => prev || "Please provide at least one valid http(s) URL.");
        return;
      }
      ...
      const newImages = extractRes.data.extraImages || [];
      setExtractedImages(prev => [...prev, ...newImages]);
      setAllImages(prev => [...prev, ...newImages]);
    } catch (error) {
      console.error("Extra image extraction error:", error);
      setError("Error extracting extra images: " + (error.response?.data?.error || error.message));
    } finally {
  ...
  const handleMetadataChange = (field, value) => {
    setMetadata(prev => ({
      ...prev,
      [field]:
        field === "publisher" || field === "designers" || field === "mechanics" || field === "artists"
      ...
      const audioPromises = currentSections.map(async (section, idx) => {
        ...
        try {
          const res = await axios.post(
            `${API_BASE}/tts`,
            { text: ttsText, voice, language, gameName },
  ...
  const handleSummarize = async () => {
    ...
    try {
      ...
      const adjustedTargetWords = Math.round(baseTargetWords * (detailPercentage / 35)); // Adjust by detail percentage (35% is baseline)
      ...
      const response = await axios.post(
        ...
        { 
          ...
          onUploadProgress: (progressEvent) => {
            ...
            setGenerateProgress(25 + Math.round(progress * 0.15)); // 25% to 40%
            setGenerateStage("Uploading data...");
          }
        }
      );
      ...
      setTimeout(() => {
        ...
        if (response.data.needsTheme) {
          ...
          const extractedSummary = extractScriptSection(response.data.summary);
          setSummary(extractedSummary);
          setEditedSummary(extractedSummary);
          setMetadata(response.data.metadata);
          if (response.data.components && Array.isArray(response.data.components)) {
            setComponents(response.data.components.map(c => ({ ...c, selected: true })));
          }
          setGenerateProgress(100);
  ...
  const handleResummarize = async () => {
    ...
    try {
      ...
      if (error.code === 'ECONNABORTED' || error.response?.status === 408) {
        errorMessage = "⏱️ Re-summarization timeout - Try reducing the detail percentage or summary length.";
      } else if (error.response?.status === 429) {
        errorMessage = "🙅 Rate limit exceeded - Please wait a moment and try again.";
      } else if (error.response?.data?.error) {
        errorMessage = error.response.data.error;
    ...
    try {
      ...
      for (let i = 0; i < newSections.length; i++) {
        try {
          ...
          setAudio(prev => ({ ...prev, [i]: audioUrl }));
        } catch (sectionError) {
          console.error(`Error generating audio for section ${i + 1}:`, sectionError);
          
          let errorMsg = `Section ${i + 1}: `;
          if (sectionError.code === 'ECONNABORTED' || sectionError.response?.status === 408) {
  ...
  const handleSaveProject = async () => {
    ...
    try {
      ...
      const response = await axios.post(`${API_BASE}/save-project`, payload, {
        headers: { "Content-Type": "application/json" }
      });
      if (response.data.status === "success") {
        alert(`Project saved! ID: ${response.data.projectId}`);
      } else {
        setError(response.data.error || "Failed to save project.");
      }
    } catch (err) {
  ...
  const handleComponentChange = (idx, field, value) => {
    const updated = [...components];
    updated[idx] = { ...updated[idx], [field]: field === "quantity" ? (value === "" ? "" : Number(value)) : value };
    setComponents(updated);
    saveComponentsToLocalStorage(updated);
  };
  ...
  const handleAddComponent = () => {
    ...
    const newComp = { name: newComponentName.trim(), quantity: newComponentQuantity ? Number(newComponentQuantity) : null, selected: true };
  ...
  return (
    <div style={{ maxWidth: 1100, margin: "40px auto", fontFamily: "sans-serif", padding: 20 }}>
        ...
        <div style={{ display: "flex", gap: "0" }}>
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              borderBottom: activeTab === "tutorial" ? "3px solid #1976d2" : "3px solid transparent",
              fontSize: "16px",
          ...
          <button
            ...
            style={{
              ...
              cursor: "pointer",
              borderBottom: activeTab === "classic" ? "3px solid #1976d2" : "3px solid transparent",
              fontSize: "16px",
      ...
      {activeTab === "tutorial" ? (
        ...
        <div>
          {error && (
            <div style={{ marginBottom: 16, padding: 12, borderRadius: 6, background: "#ffebee", border: "1px solid #ef5350", color: "#c62828" }}>
              {error}
            </div>
          )}

      <div style={{ marginBottom: "2em", padding: "20px", border: "2px solid #1976d2", borderRadius: "8px", backgroundColor: "#f5f9ff" }}>
        ...
        <div style={{ marginBottom: "1em" }}>
          <label>
            ...
            <input
              ...
              onBlur={() => {
                if (bggUrl.match(/^https?:\/\/boardgamegeek\.com\/boardgame\/\d+/)) {
                  handleExtractMetadata(bggUrl);
                }
              }}
              placeholder="Paste the BGG game page URL here"
              style={{ width: "100%", marginTop: "0.5em", marginBottom: "1em", padding: "8px", borderRadius: "4px", border: "1px solid #ccc" }}
            />
            <button
              type="button"
          ...
          <label>
            <strong>Extra URLs for Image Extraction (comma separated):</strong>
            <input
              type="text"
              value={extraImageUrls}
              onChange={e => setExtraImageUrls(e.target.value)}
              placeholder="Paste any extra URLs here, separated by commas"
              style={{ width: "100%", marginTop: "0.5em", padding: "8px", borderRadius: "4px", border: "1px solid #ccc" }}
            />
          </label>
        </div>

        {bggMetadata && (
          <div style={{ margin: "1em 0", padding: "1em", background: "#e3f2fd", borderRadius: 8 }}>
            <h3>Extracted BGG Metadata</h3>
            <ul style={{ listStyle: "none", padding: 0 }}>
              <li><b>Title:</b> {bggMetadata.title}</li>
              <li><b>Publisher:</b> {Array.isArray(bggMetadata.publisher) ? bggMetadata.publisher.join(", ") : bggMetadata.publisher || ""}</li>
              <li><b>Players:</b> {bggMetadata.player_count}</li>
              <li><b>Play Time:</b> {bggMetadata.play_time}</li>
              <li><b>Age:</b> {bggMetadata.min_age}</li>
              <li><b>Theme:</b> {bggMetadata.theme}</li>
              <li><b>Rating:</b> {bggMetadata.average_rating}</li>
              <li><b>BGG Rank:</b> {bggMetadata.bgg_rank}</li>
              <li>
                <b>Box Cover:</b><br />
                {bggMetadata.cover_image && <img src={bggMetadata.cover_image} alt="Box Cover" style={{ maxWidth: 120, marginTop: 8, borderRadius: 4, border: "1px solid #ccc" }} />}
              </li>
            </ul>
          </div>
        )}
        ...
        <div style={{ display: "flex", gap: "10px", flexWrap: "wrap" }}>
          <button
            ...
            style={{ padding: "0.5em 1.5em", fontSize: "1em", backgroundColor: "#1976d2", color: "white", border: "none", borderRadius: "4px", cursor: "pointer" }}
          >
            Start Extraction
          </button>
          <button
            onClick={async () => {
              ...
              try {
                ...
                if (extraImageUrls && extraImageUrls.trim()) await handleExtractExtraImages();
            ...
            disabled={(!bggUrl.trim() && !extraImageUrls.trim()) || isExtractingImages}
            style={{
              padding: "0.5em 1.5em",
              fontSize: "1em",
              backgroundColor: (!bggUrl.trim() && !extraImageUrls.trim()) || isExtractingImages ? "#b0bec5" : "#ff9800",
              color: "white",
              border: "none",
              borderRadius: "4px",
              cursor: (!bggUrl.trim() && !extraImageUrls.trim()) || isExtractingImages ? "not-allowed" : "pointer"
            }}
          >
      ...
      >
        <div style={{ marginBottom: 8, fontWeight: 600 }}>Drop your PDF rulebook here, or click to browse</div>
        <div style={{ fontSize: 12, color: "#666" }}>Only .pdf files are supported</div>
        <input type="file" accept="application/pdf" ref={fileInputRef} onChange={handleFileChange} style={{ display: "none" }} />
      </div>

      {loading && (
        <div style={{ marginBottom: 16, padding: 12, borderRadius: 6, background: "#f5f5f5", border: "1px solid #ddd" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <label>
          <b>Game Name:</b>{" "}
          <input type="text" value={gameName} onChange={e => setGameName(e.target.value)} placeholder="Enter the game name" style={{ width: "calc(100% - 110px)", marginRight: 10 }} />
        </label>
      </div>
      ...
      <div style={{ marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(250px, 1fr))", gap: 10 }}>
          <input
            type="text"
            value={Array.isArray(metadata.publisher) ? metadata.publisher.join(", ") : metadata.publisher || ""}
            onChange={e => handleMetadataChange("publisher", e.target.value)}
            placeholder="Publisher (e.g., Bombyx)"
          />
          <input type="text" value={metadata.player_count} onChange={e => handleMetadataChange("player_count", e.target.value)} placeholder="Player Count (e.g., 2-4)" />
          <input type="text" value={metadata.play_time} onChange={e => handleMetadataChange("play_time", e.target.value)} placeholder="Game Length (e.g., 30-60 min)" />
          <input type="text" value={metadata.min_age} onChange={e => handleMetadataChange("min_age", e.target.value)} placeholder="Minimum Age (e.g., 10+)" />
          <input
            type="text"
            value={Array.isArray(metadata.designers) ? metadata.designers.join(", ") : metadata.designers || ""}
            onChange={e => handleMetadataChange("designers", e.target.value)}
            placeholder="Designers (comma separated)"
          />
          <input
            type="text"
            value={Array.isArray(metadata.artists) ? metadata.artists.join(", ") : metadata.artists || ""}
            onChange={e => handleMetadataChange("artists", e.target.value)}
            placeholder="Artists (comma separated)"
          />
          <input
            type="text"
            value={Array.isArray(metadata.mechanics) ? metadata.mechanics.join(", ") : metadata.mechanics || ""}
            onChange={e => handleMetadataChange("mechanics", e.target.value)}
            placeholder="Mechanics (comma separated)"
          />
          <input type="text" value={metadata.theme} onChange={e => handleMetadataChange("theme", e.target.value)} placeholder="Theme (e.g., Fantasy)" />
        </div>
      </div>

      {pdfPageImages.length > 0 && (
        <div style={{ margin: "24px 0", padding: 20, border: "2px solid #9c27b0", borderRadius: 8, background: "#faf5ff" }}>
          <h2>PDF Page Previews</h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(120px, 1fr))", gap: 10 }}>
            {pdfPageImages.map((img, idx) => (
              <div key={img.id || idx} style={{ background: "#fff", border: "1px solid #eee", borderRadius: 6, padding: 6, textAlign: "center" }}>
                <img
                  src={displayImageUrl(img)}
                  alt={`PDF page ${idx + 1}`}
                  onClick={() => openLightbox(displayImageUrl(img), `Page ${idx + 1}`)}
                  style={{ width: "100%", height: 160, objectFit: "contain", borderRadius: 6, border: "1px solid #ddd", cursor: "pointer", background: "#f8f8f8" }}
                />
                <div style={{ fontSize: 12, marginTop: 6, color: "#666" }}>{img.name || `Page ${idx + 1}`}</div>
                <div style={{ fontSize: 10, color: "#999" }}>Click to enlarge</div>
              </div>
            ))}
          </div>
        </div>
      ...
      <div style={{ margin: "24px 0", padding: 20, border: "2px solid #ff5722", borderRadius: 8, background: "#fff8f5" }}>
        <h2>Components</h2>
        {components.length === 0 ? (  
          <div style={{ color: "#888" }}>  
            No components extracted yet. Upload a PDF or provide sources to extract from.  
          </div>  
        ) : (  
          <>  
            <div style={{ marginBottom: 12, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ color: "#555" }}>
                {components.filter(c => c?.selected !== false).length} components found.
              </span>

              {!componentsConfirmed && (
                <button type="button" onClick={confirmComponents} style={{ padding: "6px 12px" }}>
                  Confirm components
                </button>
              )}
              ...
              <button
                ...
                onClick={() => {
                  ...
                  const matchedComps = improvedMatchComponentsToImages(components, allImages);
                  const newOverrides = {};
                  const newMulti = {};
                  ...
                  matchedComps.forEach((comp, idx) => {
                    if (comp.autoAssigned && comp.suggestedImage) {
                      const imageKey = comp.suggestedImage.id || comp.suggestedImage.path;
                  ...
                  setCompImageOverrides(prev => ({...prev, ...newOverrides}));
                  setCompImageMulti(prev => ({...prev, ...newMulti}));
                  
                  setError(`✨ Auto-matched ${autoAssignedCount} components with high confidence!`);
                  setTimeout(() => setError(""), 3000);
                }}
                style={{ padding: "6px 12px", background: "#10b981", color: "white", border: "none", borderRadius: 4, cursor: "pointer" }}
              ...
              <button
                ...
                style={{ padding: "6px 12px", background: "#0ea5e9", color: "white", border: 0, borderRadius: 6, cursor: "pointer" }}
              ...
              <table style={{ width: "100%", borderCollapse: "collapse" }}>
                <thead>
                  <tr>
                    <th style={{ textAlign: "left", padding: 8, border: "1px solid #c8e6c9" }}>Component</th>
                    <th style={{ textAlign: "left", padding: 8, border: "1px solid #c8e6c9" }}>Qty</th>
                    <th style={{ textAlign: "left", padding: 8, border: "1px solid #c8e6c9" }}>Match / Selection</th>
                    <th style={{ textAlign: "left", padding: 8, border: "1px solid #c8e6c9" }}>Confidence</th>
                    <th style={{ textAlign: "left", padding: 8, border: "1px solid #c8e6c9" }}>Reason</th>
                    <th style={{ textAlign: "left", padding: 8, border: "1px solid #c8e6c9" }}>Alternatives</th>
                  </tr>
                </thead>
                ...
                <tbody>
                  {matchedComponents.map((comp, idx) => (
                    <tr key={idx}>
                      <td style={{ padding: 8, border: "1px solid #c8e6c9" }}>{comp.name}</td>
                      <td style={{ padding: 8, border: "1px solid #c8e6c9" }}>{comp.quantity ?? ""}</td>

                      <td
                        style={{ padding: 8, border: "1px solid #c8e6c9", verticalAlign: "top" }}
                      ...
                      >
                        <div style={{ marginBottom: 8 }}>
                          {comp.suggestedImage ? (
                            <img
                              ...
                              style={{ maxWidth: 60, maxHeight: 60, borderRadius: 4, border: "1px solid #ccc", objectFit: "contain", background: "#f8f8f8" }}
                            />
                          ) : (
                            <span style={{ color: "#aaa" }}>Drop an image here</span>
                          )}
                        </div>
                        ...
                        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
                          {(comp.selectedImages || []).map(img => (
                            <img
                              key={(img && (img.id || img.path)) || Math.random()}
                              src={displayImageUrl(img)}
                              alt={(img && (img.description || img.name)) || "selected"}
                              style={{
                                width: 60,
                                height: 60,
                                objectFit: "contain",
                      ...
                      <td style={{ padding: 8, border: "1px solid #c8e6c9" }}>{comp.matchReason}</td>
                      <td style={{ padding: 8, border: "1px solid #c8e6c9" }}>
                        {comp.alternativeMatches && comp.alternativeMatches.length > 0 ? (
                          comp.alternativeMatches.map((alt, i) => (
                            <img
                              key={i}
                              src={displayImageUrl(alt.image)}
                              alt={alt.image.name || "Alternative"}
                              style={{ maxWidth: 40, maxHeight: 40, marginRight: 4, borderRadius: 3, border: "1px solid #eee" }}
                            />
                          ))
                        ) : (
              ...
              {pickerOpenIdx !== null && (
                ...
                >
                  ...
                  >
                    ...
                    {/* Picker actions */}
                    <div style={{ display: "flex", gap: 8, flexWrap: "wrap", padding: "12px 16px", borderBottom: "1px solid #eee" }}>
                      <button
                        type="button"
                        onClick={() => {
                          const currentIdx = pickerOpenIdx;
                          closeImagePicker();
                          setTimeout(() => openWebSearchForComponent(currentIdx), 0);
                        }}
                        style={{
                          padding: "8px 12px",
                          background: "#0ea5e9",
                    ...
                    <div style={{ padding: 16, overflow: "hidden" }}>
                      ...
                      >
                        {allImages
                          .filter(img => img && img.path && !excludedImageIds.has(imgKey(img)))
                          .map(img => {
                            const key = imgKey(img);
                            ...
                            return (
                              <div
                                key={key}
                                style={{
                                  border: selected ? "2px solid #155eef" : "1px solid #e5e7eb",
                                  borderRadius: 10,
                                  padding: 8,
                                  background: selected ? "#eff4ff" : "white",
                                  boxShadow: selected ? "0 0 0 4px rgba(21,94,239,0.08)" : "none"
                                }}
                              >
                                <div style={{ position: "relative" }}>
                                  <img
                                    src={displayImageUrl(img)}
                                    alt={img.description || img.name || img.filename || "image"}
                                    style={{
                                      width: "100%",
                                      height: THUMB_SIZE,
                                      objectFit: "contain",
                                ...
                                <div style={{ display: "flex", gap: 8, marginTop: 8 }}>
                                  ...
                                  <button
                                    ...
                                    style={{
                                      padding: "6px 10px",
                                      borderRadius: 6,
                                      border: primary ? "1px solid #155eef" : "1px solid #d1d5db",
                                      background: primary ? "#eff4ff" : "white",
              ...
              {webModalOpen && (
                ...
                >
                  <div
                    ...
                    onClick={e => e.stopPropagation()}
                  >
                    <div style={{ display: "flex", alignItems: "center", padding: 16, borderBottom: "1px solid #eee", gap: 12, flexWrap: "wrap" }}>
                      <h4 style={{ margin: 0, fontSize: 18, fontWeight: 600, flex: 1 }}>
                      ...
                      <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                        <input
                          ...
                          style={{ minWidth: 240, padding: "6px 8px", borderRadius: 6, border: "1px solid #d1d5db" }}
                        />
                        <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          Pages:
                          <select value={webPageLimit} onChange={e => setWebPageLimit(Number(e.target.value))}>
                        ...
                        <button
                          ...
                          disabled={webLoading}
                          style={{ padding: "8px 12px", background: "#0ea5e9", color: "white", border: 0, borderRadius: 6, cursor: "pointer" }}
                        >
                    ...
                    <div style={{ padding: 16, borderBottom: "1px solid #eee", display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
                      <div>
                        Mode:{" "}
                        <strong>{webModalMode === "single" ? "Single Component" : "Batch (assign to multiple components from one fetch)"}</strong>
                      </div>

                      {webModalMode === "single" && webTargetIdx != null && (
                        <div style={{ color: "#334155" }}>
                          Target: <strong>{matchedComponents[webTargetIdx]?.name || `Component ${webTargetIdx + 1}`}</strong>
                        </div>
                      )}
                      ...
                      {webModalMode === "batch" && (
                        <label style={{ display: "flex", alignItems: "center", gap: 6 }}>
                          ...
                          <select
                            ...
                            onChange={e => setWebBatchTargetIdx(Number(e.target.value))}
                          >
                            {components.map((c, i) => (
                    ...
                    <div style={{ padding: 16, overflow: "hidden" }}>
                      {webLoading ? (
                        ...
                        <div style={{ color: "#64748b" }}>
                          No results yet. Enter a query and click Search. Tip: the query is prefilled as “{gameName} + component” for single component mode.
                        </div>
                      ) : (
                        <div
                          style={{
                            ...
                            return (
                              <div
                                ...
                                style={{
                                  ...
                                  boxShadow: selected ? "0 0 0 4px rgba(14,165,233,0.08)" : "none"
                                }}
                              >
                                <div style={{ position: "relative" }}>
                                  <img
                                    src={displayImageUrl(img)}
                                    alt={img.description || img.name || "image"}
                                    style={{ width: "100%", height: 140, objectFit: "contain", borderRadius: 8, display: "block", background: "#f8f8f8" }}
                                    onClick={() => toggleWebSelect(img)}
                                    onDoubleClick={() => openLightbox(displayImageUrl(img), img.name || "Image")}
                                  />
                                  {selected && (
                                    <div
                                      style={{
                                ...
                                <div style={{ marginTop: 8, fontSize: 12, color: "#334155", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                  {img.name || "Image"}
                                </div>
                              </div>
                            );
                    ...
                    <div style={{ display: "flex", justifyContent: "flex-end", gap: 8, padding: 16, borderTop: "1px solid #eee" }}>
                      {webModalMode === "single" && webTargetIdx != null && (
                        <button  
                          type="button"  
                          onClick={() => handleUseSelectedSingle(webTargetIdx)}  
                          disabled={webSelected.length === 0}  
                          style={{ padding: "8px 12px", borderRadius: 8, border: 0, background: "#0ea5e9", color: "white", cursor: "pointer" }}  
                      ...
                      {webModalMode === "batch" && (
                        <button
                          type="button"
                          onClick={() => applyWebSelectionToComponent(webBatchTargetIdx)}
                          disabled={webSelected.length === 0 || webBatchTargetIdx == null}
                          style={{ padding: "8px 12px", borderRadius: 8, border: 0, background: "#0ea5e9", color: "white", cursor: "pointer" }}
      ...
      {allImages.length > 0 && (
        <div style={{ margin: "24px 0", padding: 20, border: "2px solid #009688", borderRadius: 8, background: "#f0fffc" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <h2 style={{ margin: 0 }}>Images</h2>
            <button onClick={handleFetchBGGImages} disabled={fetchingBGG} style={{ padding: "8px 16px", borderRadius: 4, border: "none", background: "#009688", color: "#fff", cursor: "pointer" }}>
                ...
                >
                  ...
                  >
                    <img
                      ...
                      draggable
                      onDragStart={e => e.dataTransfer.setData("application/image-id", key)}
                      onDoubleClick={() => openLightbox(displayImageUrl(img), img.name || "Image")}
                      style={{ maxWidth: "100%", maxHeight: "100%", objectFit: "contain" }}
                    />
                  </div>
                  <div style={{ marginTop: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                    <span style={{ fontSize: 12, color: "#555", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{img.name || "Image"}</span>
                    <label style={{ fontSize: 12, color: "#333", display: "flex", alignItems: "center", gap: 4 }}>
                      <input type="checkbox" checked={!excludedImageIds.has(key)} onChange={() => toggleExcluded(key)} />
                      Use
                    </label>
                  </div>
                </div>
          ...
          <div style={{ marginTop: 12, display: "flex", gap: 8 }}>
            <button onClick={applyImageSelection} style={{ padding: "6px 12px" }}>Update Images</button>
            <button onClick={() => setExcludedImageIds(new Set(allImages.map(i => i.id || i.path)))} style={{ padding: "6px 12px" }}>Select None</button>
            <button onClick={() => setExcludedImageIds(new Set())} style={{ padding: "6px 12px" }}>Select All</button>
          </div>
        </div>
      )}

      <section style={{ marginTop: "2rem", padding: "1rem", border: "1px solid #eee", borderRadius: "8px" }}>
        <h3>Storyboard</h3>
        <div style={{ display: "flex", gap: "1rem", alignItems: "center", marginBottom: "0.75rem" }}>
          ...
          <label>
            Voice:{" "}
            <select value={storyVoice} onChange={e => setStoryVoice(e.target.value)}>
          ...
          <button onClick={handleGenerateStoryboard} disabled={isGeneratingStoryboard}>
            {isGeneratingStoryboard ? "Generating…" : "Generate Storyboard"}
          </button>
          <button onClick={handleCopyStoryboardJson} disabled={!storyboard} title="Copy storyboard JSON to clipboard">
            Copy JSON
          </button>
          {storyboard && <span style={{ color: "#555" }}>Estimated total: {Math.round(storyboard.totalDurationSec)}s</span>}
          {copySbFeedback && <span style={{ marginLeft: 8, color: copySbFeedback === "Copied!" ? "green" : "crimson" }}>{copySbFeedback}</span>}
        </div>

        {storyboard && (
          <div>
            {storyboard.scenes.map(scene => (
              <div key={scene.id} style={{ marginBottom: "1rem", padding: "0.75rem", background: "#fafafa", borderRadius: "6px" }}>
                <div style={{ fontWeight: 600, marginBottom: "0.5rem" }}>{storyLang === "fr" ? scene.titleFr || scene.title : scene.title}</div>
                {(scene.segments || []).map(seg => (
                  <div key={seg.id} style={{ display: "flex", gap: "0.75rem", alignItems: "center", marginBottom: "0.5rem" }}>
                    {seg.image && (
                      <img
                        src={displayImageUrl(seg.image)}
                        alt={seg.image.name || "segment"}
                        style={{ width: 96, height: 64, objectFit: "cover", borderRadius: "4px", border: "1px solid #ddd" }}
                      />
                    )}
                    <div style={{ flex: 1 }}>
                      <div style={{ color: "#333" }}>{storyLang === "fr" ? seg.textFr : seg.textEn}</div>
                      <div style={{ fontSize: 12, color: "#777", marginTop: 4 }}>Duration: {seg.durationSec}s</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div style={{ margin: "32px 0", padding: 20, border: "2px solid #3f51b5", borderRadius: 8, background: "#f5f6ff" }}>
        <h2>Script</h2>
        <div style={{ marginBottom: 10, color: "#666" }}>
          {rulebookText ? `Rulebook text loaded (${countWords(rulebookText)} words).` : "Paste rulebook text below or upload a PDF to extract it automatically."}
        </div>
        <textarea
          value={rulebookText}
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 10, marginBottom: 12 }}>
          ...
          <div style={{ display: "flex", alignItems: "center", gap: 8, marginLeft: loading ? 60 : 0 }}>
            <label style={{ fontWeight: 600 }}>Detail %:</label>
            <input type="range" min="0" max="100" value={detailPercentage} onChange={e => setDetailPercentage(Number(e.target.value))} />
            <span>{detailPercentage}%</span>
            <button onClick={handleResummarize} style={{ padding: "8px 16px", borderRadius: 4, border: "none", background: "#8e24aa", color: "#fff", cursor: "pointer" }}>
              Re-summarize
            </button>
          </div>
          <button onClick={handleSaveSummary} style={{ padding: "8px 16px", borderRadius: 4, border: "none", background: "#009688", color: "#fff", cursor: "pointer" }}>
            Save Script
          </button>
          <button onClick={handleSaveAndContinue} style={{ padding: "8px 16px", borderRadius: 4, border: "none", background: "#ff5722", color: "#fff", cursor: "pointer" }}>
        ...
        <textarea
          ...
          rows={16}
          style={{ width: "100%", borderRadius: 6, border: "1px solid #ccc", padding: 10 }}
        />
        ...
        {sections.length > 0 && (
          <div style={{ marginTop: 16 }}>
            ...
            {sections.map((sec, idx) => (
              <div key={idx} style={{ marginBottom: 16, padding: 12, background: "#fff", border: "1px solid #e0e0e0", borderRadius: 6 }}>
                <div style={{ marginBottom: 8, fontWeight: 600 }}>Section {idx + 1}</div>
                <div style={{ marginBottom: 8, color: "#444" }}>
                  <ReactMarkdown>{sec}</ReactMarkdown>
                </div>
                {audioLoading[idx] ? (
                  <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
                    <ClipLoader size={16} color="#ff5722" /> Generating audio...
                  </div>
                ) : audio[idx] ? (
                  <audio controls src={audio[idx]} style={{ width: "100%" }} />
                ) : (
                  <div style={{ color: "#999" }}>No audio generated for this section yet.</div>
                )}
              </div>
            ))}
          </div>
      ...
      <div style={{ margin: "24px 0", textAlign: "right" }}>
        <button onClick={handleSaveProject} style={{ padding: "10px 18px", borderRadius: 6, border: "none", background: "#1976d2", color: "#fff", cursor: "pointer" }}>
          Save Project
        </button>
      </div>
        </div>
