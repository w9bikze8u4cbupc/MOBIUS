...
export default function ImageSelectModal({
  ...
  const withScores = useMemo(() => {
    return (images || []).map(img => ({
      ...img,
      __score: scoreImage(img),
      __likely: isLikelyComponent(img),
    }));
  }, [images]);
    ...
    <div style={S.overlay} role="dialog" aria-modal="true">
      <div style={S.body}>
        ...
        <div style={S.header}>
          <div style={S.tabs}>
            <button
              style={tabStyle(filterTab === 'all')}
              onClick={() => setFilterTab('all')}
            >
              All
            </button>
            <button
              style={tabStyle(filterTab === 'components')}
              onClick={() => setFilterTab('components')}
            >
              Components
            </button>
            <button
              style={tabStyle(filterTab === 'other')}
            ...
            return (
              ...
              >
                ...
                <div style={S.meta}>
                  ...
                  <span> · {img.type || ''}</span>
                  {typeof img.__score === 'number' ? (
                    <span> · ★ {img.__score.toFixed(1)}</span>
                  ) : null}
                </div>
              </button>
            );
