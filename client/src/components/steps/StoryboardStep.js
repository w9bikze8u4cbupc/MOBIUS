        ...
        return <article key={scene.id} style={{ border: '1px solid #d0d7de', borderRadius: 8, padding: 12, marginBottom: 12 }}>
          ...
          <div><strong>Selected visual assets:</strong> {selectedIds.length ? selectedIds.map((assetId) => {
            ...
            return <div key={assetId} style={{ margin: '6px 0' }}><VisualAsset projectId={projectId} asset={imageById.get(assetId)} assetId={assetId} />
              <label>role <select aria-label={`Role for ${assetId} in ${scene.id}`} value={role} onChange={(event) => onUpdateScene(scene.id, { visualPlan: { ...plan, assetAssignments: [...(plan.assetAssignments || []).filter((item) => item.assetId !== assetId), { ...assignment, assetId, role: event.target.value, componentId: event.target.value === 'primary' ? (plan.primaryComponentRefs || []).includes(assignment.componentId) ? assignment.componentId : null : event.target.value === 'supporting' ? (plan.supportingComponentRefs || []).includes(assignment.componentId) ? assignment.componentId : null : null }] } })}>{allowedRoles.map((option) => <option key={option} value={option}>{option.replace('_', ' ')}</option>)}</select></label>
              {(['primary', 'supporting'].includes(role)) && (() => {
                const componentOptions = role === 'primary' ? (plan.primaryComponentRefs || []) : (plan.supportingComponentRefs || []);
                return componentOptions.length > 0 ? <label>component <select aria-label={`Component for ${assetId} in ${scene.id}`} value={assignment.componentId || ''} onChange={(event) => onUpdateScene(scene.id, { visualPlan: { ...plan, assetAssignments: [...(plan.assetAssignments || []).filter((item) => item.assetId !== assetId), { ...assignment, assetId, role, componentId: event.target.value || null }] } })}><option value="">choose explicitly</option>{componentOptions.map((componentRef) => <option key={componentRef} value={componentRef}>{componentRequirementLabel(plan, componentRef)}</option>)}</select></label> : null;
              })()}
