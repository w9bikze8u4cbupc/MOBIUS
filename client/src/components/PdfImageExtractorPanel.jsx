...
import { extractActionsByUrl, sortImagesForPicking } from '../api/extractActions';
...
export default function PdfImageExtractorPanel({ onUse }) {
  ...
  const runActions = async () => {
    ...
    try {
      ...
      if (actionImages.length === 0) {
        setError('No "Actions" images found. Try another PDF or adjust keywords.');
        return;
      }
  ...
  return (
    <div style={{ border: '1px solid #333', borderRadius: 8, padding: 12, background: '#0e0e0e' }}>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
        ...
        <button 
          ...
          style={{ background: '#f39c12', color: '#000', padding: '8px 12px', borderRadius: 6 }}
        >
        ...
        <button 
          onClick={runActions} 
          disabled={!pdfUrl || loading || loadingActions} 
          style={{ background: '#e74c3c', color: '#fff', padding: '8px 12px', borderRadius: 6 }}
        >
          {loadingActions ? 'Finding Actions…' : 'Choose Actions Image'}
        </button>
        {jobId ? <span style={{ color: '#bbb' }}>Last job: {jobId}</span> : null}
      </div>
      ...
      <ImageSelectModal
        ...
        onConfirm={handleUse}
        title={extractionType === 'actions' ? 'Choose Action Image' : 'Choose Images'}
      />
    </div>
  );
}