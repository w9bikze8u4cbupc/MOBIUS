...
function ImageMatcher({
  ...
  const normalizeImage = useCallback((img, index) => {
    ...
    if (img && typeof img === 'object') {
      return {
        ...
        preview: String(
          img.preview || img.thumbnail || img.path || img.url || img.src || ''
        ).trim(),
      };
    }
    return {
    ...
    async (comps, imgs, callback) => {
      ...
      try {
        ...
        if (data && data.success) {
          ...
          throw new Error(
            (data && (data.error || data.message)) || 'Unknown server error'
          );
        }
      } catch (err) {
        if (!isMountedRef.current) return;
  ...
  const handleDragEnd = useCallback(result => {
    ...
    if (
      source.droppableId.startsWith('component-') &&
      ...
      >
        ...
        >
          ...
          <div style={{ display: 'flex', gap: '10px', alignItems: 'center' }}>
            ...
            >
              {loading
                ? 'Saving...'
                : `Save Matches (${Object.keys(matches).length})`}
            </button>
          </div>
        </div>
                            ...
                            {(provided, snapshot) => (
                              ...
                              >
                                <img
                                  ...
                                  onError={e =>
                                    handleImageError(e, matchedImage.name)
                                  }
                                />
                                <span
        ...
        {process.env.NODE_ENV === 'development' && (
          ...
          >
            ...
            Images: {extractedImages.length}, Available:{' '}
            {getAvailableImages().length}, Matches:{' '}
            {Object.keys(matches).length}, Retries: {retryCount}
          </div>
        )}
      </div>
    </DragDropContext>
