...
async function handleExtractMetadata(bggUrl) {
...
function App() {
  ...
  const handleGenerateStoryboard = async () => {
    try {
      ...
      const res = await axios.post(
        `${API_BASE}/api/generate-storyboard`,
        payload,
        {
  ...
  const handleFetchBGGImages = async () => {
    ...
    try {
      ...
      if (response.data.success) {
        const bggImgs = Array.isArray(response.data.images)
      ...
      setTimeout(() => {
        ...
        if (response.data.needsTheme) {
          ...
          if (
            ...
            setComponents(
              response.data.components.map(c => ({ ...c, selected: true }))
            );
          }
          setGenerateProgress(100);
  ...
  return (
    ...
    >
      ...
      {showDevTest && (
        <DevTestPage onClose={() => setShowDevTest(false)} />
      )}
