...
const improvedMatchComponentsToImages = (components, images) => {
  console.log('Starting improved matching with:', {
  ...
  function checkSynonymMatch(componentName, imageName, imageDesc) {
    ...
    for (const [key, syns] of Object.entries(synonyms)) {
      ...
      const imageHasSynonym = syns.some(
        syn => imgNameLower.includes(syn) || imgDescLower.includes(syn)
      );
  ...
  return components.map((component, compIndex) => {
    ...
    images.forEach((img, imgIndex) => {
      ...
      if (
        ...
        imageDesc.includes(componentName)
      ) {
      ...
      // 2. Synonym matching
      const synonymScore = checkSynonymMatch(
        component.name,
        imageName,
        imageDesc
      );
      ...
      colors.forEach(color => {
        if (
          ...
          (imageName.includes(color) || imageDesc.includes(color))
        ) {
      ...
      console.log(
        `  Image ${imgIndex} (${imageName}): score=${score.toFixed(2)}, reasons=[${reasons.join(', ')}]`
      );
    });

    console.log(
      `  Best match: ${bestMatch ? bestMatch.name : 'none'} (score: ${bestScore.toFixed(2)}, reason: ${matchReason})`
    );
