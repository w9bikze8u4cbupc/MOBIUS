...
function App() {
  ...
  const handleImageSelection = (images, context) => {
    ...
    if (context.extractionType === 'actions') {
      console.log('User selected action images from pages:', 
    ...
    <div className="app">
      ...
      {selectedImages.length > 0 && (
        <div style={{ marginTop: 20 }}>
          <h3>Selected Images ({extractionContext?.extractionType || 'general'})</h3>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 10 }}>
            {selectedImages.map((img, i) => (
              <div key={i} style={{ border: '1px solid #ccc', padding: 10, borderRadius: 8 }}>
                <img src={img.url} alt={img.name} style={{ width: '100%', height: 150, objectFit: 'contain' }} />
                <div style={{ fontSize: 12, marginTop: 5 }}>
                  <strong>{img.name}</strong><br />
                  {img.width}×{img.height}px<br />
                  {img.page && `Page ${img.page}`}<br />
                  {img.isActionImage && <span style={{ color: '#e74c3c' }}>🎯 Action Image</span>}
                </div>
              </div>
            ))}
          </div>
