...
function testNegativeCases() {
  ...
  NEGATIVE_LINES.forEach((line, index) => {
    ...
    console.log(`${passed ? '✅' : '❌'} Test ${index + 1}: "${line}" -> ${result.length} components`);
    if (!passed) {
  ...
  console.log(`${threatPassed ? '✅' : '❌'} Threat token correctly excluded: ${threatResult.length} components found`);
  if (!threatPassed) {
