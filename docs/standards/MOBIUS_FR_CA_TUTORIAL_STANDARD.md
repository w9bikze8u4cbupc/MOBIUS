...
- Applicability is evidence-based: a domain is `APPLICABLE` only through positive source evidence or reviewed project data, `NOT_APPLICABLE` only through source-grounded determination, and otherwise `UNKNOWN`. Generic coverage must not require a first-player, cost, or round mechanic merely because another game has one.
- Before Cockpit review, a source-grounded candidate rejected only for missing structure receives one validator-guided corrective synthesis using the original evidence, candidate, and exact validator issues. If evidence is insufficient, production may make one bounded neighboring/heading retrieval expansion. Both paths are cacheable, evidence-only, and must terminate rather than loop.
- Domain validators express learner-relevant rule requirements, not renderer convenience: turn order, first-player selection, and payment rules may be complete without invented component bindings or arbitrary state-after fields.
