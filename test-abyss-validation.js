  ...
  try {
    ...
    if (components.length > 0) {
      ...
      components.forEach((comp, i) => {
        console.log(
          `   ${i + 1}. ${comp.name} ${comp.quantity ? `(${comp.quantity})` : '(no qty)'}${comp.note ? ` [${comp.note}]` : ''}`,
        );
      });
...
function analyzeComponentDiversity(components) {
  ...
  components.forEach((comp) => {
    ...
    if (name.includes('pearl') || name.includes('coin') || name.includes('money'))
      types.add('currency');
...
function validateAbyssImprovement(componentTypes, totalComponents) {
  ...
  console.log(
    `   📈 New System: ${componentTypes.size} types (${Array.from(componentTypes).join(', ')})`,
  );
  ...
  if (componentTypes.size >= 8) {
    ...
    console.log(
      `   🎯 Detected ${componentTypes.size}/${expectedTypes.length} expected types (${improvementScore.toFixed(0)}%)`,
    );
...
function analyzeAbyssResults(apiData) {
  ...
  if (components && components.length > 0) {
    ...
    components.forEach((comp, i) => {
      console.log(
        `   ${i + 1}. ${comp.name || 'Unknown'} ${comp.quantity ? `(${comp.quantity})` : ''}${comp.note ? ` [${comp.note}]` : ''}`,
      );
    });
