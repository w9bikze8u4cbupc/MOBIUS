...
async function runHardeningGoldenTests() {
  ...
  for (const game of testGames) {
    ...
    try {
      ...
      const mediumConfidenceCount = harvestResult.filter(img => img.confidence === 'Medium').length;
      ...
      console.log(`   ⭐ Medium confidence: ${mediumConfidenceCount} (expected: ≥${game.expected.minMediumConfidence})`);
      ...
      if (testPassed) {
        ...
        console.log(`      Components: ${componentCount} >= ${Math.max(1, game.expected.components * 0.5)}`);
        console.log(`      Images: ${imageCount} > 0`);
        console.log(`      Medium confidence: ${mediumConfidenceCount} >= ${game.expected.minMediumConfidence}`);
      }
  ...
  const canonicalTest = canonicalizeImageUrl('https://example.com/image.jpg?utm_source=test&width=100');
  ...
  const confidenceTest = confidenceBand({ providerWeight: 0.85, proximityScore: 0.9, sizeScore: 0.8, qualityFocus: 0.7 });
...
runHardeningGoldenTests().then(success => {
