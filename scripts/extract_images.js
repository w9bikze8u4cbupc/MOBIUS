...
async function main() {
  ...
  try {
    if (pdfimagesPath) {
      ...
      // pdfimages names files like prefix-000.png, prefix-001.png...
  ...
  for (let i = 0; i < extractedFiles.length; i++) {
    ...
    // If the filename differs (pdfimages might already use png), rename/copy to consistent master path
    try {
    ...
    // Process image (trim/crop/normalize) and produce derivatives
    try {
      const processed = await processImage(masterPath, {
        outDir: imagesDir,
        basename: `${baseName}-${seq}`,
        generateWebP: true,       // produce web-friendly derivative for video workflow
        generateThumbnail: true,
        losslessMaster: true      // keep PNG master
      });
