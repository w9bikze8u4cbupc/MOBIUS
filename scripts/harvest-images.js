...
import { loadGameProfile, normalizeComponentName, isSupplyOnly } from '../src/utils/game-profiles.js';
...
const COMPONENT_SECTION_RE = /(components?|contents|setup|material|spielmaterial|contenu|componentes|componenti|matériel|composants|contenidos|materiale)/i;
  ...
  for (const u of extraUrls) {
    ...
    try {
      ...
      $('img').each((_, el) => {
        ...
        // Skip if too small
        const tooSmall = (bestWidth > 0 && bestWidth < MIN_SIZE) || (bestHeight > 0 && bestHeight < MIN_SIZE);
        if (tooSmall) {
        ...
        if (verbose) console.log(`✅ Collected image: ${abs} (boost: ${vicinityBoost}, size: ${bestWidth}x${bestHeight})`);
      });
...
export async function harvestAllImages(ctx) {
  ...
  for (const provider of PROVIDERS) {
    ...
    try {
      ...
      if (verbose) console.log(`📊 Provider ${provider.name}: ${imgs.length} images (${endTime - startTime}ms)`);
    } catch (e) {
  ...
  const headers = {
    ...
    'X-Images-Provider-Counts': Object.entries(providerCounts).map(([k,v]) => `${k}:${v}`).join(',')
  };
...
function dedupeAndRank(imgs, labels, verbose = false) {
  ...
  for (const img of imgs) {
    ...
    if (!prev || img.vicinityBoost > prev.vicinityBoost || 
...
if (import.meta.url === `file://${process.argv[1]}`) {
  ...
  if (!gameTitle && extraUrls.length === 0) {
    console.log('Usage: node harvest-images.js [--title "Game Name"] [--extra <url1>] [--extra <url2>] --labels "Label1,Label2" [--verbose]');
    process.exit(1);
  }
