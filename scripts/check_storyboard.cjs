#!/usr/bin/env node
/**
 * MOBIUS Storyboard Contract Validator (Phase E2/E3)
 *
 * Usage:
 *   node scripts/check_storyboard.cjs --input path/to/storyboard.json \
 *     --contract docs/spec/storyboard_contract_v1.1.0.json \
...
function validateStoryboard(storyboard, contract) {
  ...
  storyboard.scenes.forEach((scene, idx) => {
    ...
    // Type enum is enforced by contract, but we assert some logical expectations
    if (!scene.type) {
      errors.push(`${prefix}: type must be set`);
    }

    // Duration
    if (typeof scene.durationSec !== "number" || scene.durationSec < 1.0) {
      errors.push(`${prefix}: durationSec must be >= 1.0`);
    }

    // Linking
    if (idx === 0 && scene.prevSceneId !== null) {
      errors.push(`${prefix}: first scene must have prevSceneId = null`);
    }
    if (idx === storyboard.scenes.length - 1 && scene.nextSceneId !== null) {
      errors.push(`${prefix}: last scene must have nextSceneId = null`);
    }
    if (idx > 0 && scene.prevSceneId && !sceneIds.has(scene.prevSceneId)) {
      errors.push(`${prefix}: prevSceneId '${scene.prevSceneId}' does not reference a known scene`);
    }
    if (idx < storyboard.scenes.length - 1 && scene.nextSceneId && !sceneIds.has(scene.nextSceneId)) {
      errors.push(`${prefix}: nextSceneId '${scene.nextSceneId}' does not reference a known scene`);
    }
      ...
      for (const visual of scene.visuals) {
        ...
        if (!visual.placement) {
          ...
          if (![x, y, width, height].every((v) => typeof v === "number")) {
            ...
            // If normalized, warn if outside safe area
            if (
              x >= 0 && x <= 1 &&
              y >= 0 && y <= 1 &&
              width >= 0 && width <= 1 &&
              ...
              if (x < SAFE_MARGIN || y < SAFE_MARGIN) {
                warnings.push(`${vPrefix}: near top/left edge (check safe area)`);
              }
              if (x + width > 1 - SAFE_MARGIN || y + height > 1 - SAFE_MARGIN) {
                warnings.push(`${vPrefix}: near bottom/right edge (check safe area)`);
              }
            }
          }
        }
      ...
      for (const overlay of scene.overlays) {
        ...
        if (!overlay.placement) {
          ...
          if (![x, y, width, height].every((v) => typeof v === "number")) {
            ...
            warnings.push(`${oPrefix}: placement appears to be non-normalized; check renderer expectations`);
          }
        }
...
(function main() {
  ...
  const contractPath =
    args.contract || path.join("docs", "spec", "storyboard_contract_v1.1.0.json");
  const junitPath = args.junit;

  if (!inputPath) {
    console.error(
      "[STORYBOARD] Usage: node scripts/check_storyboard.cjs --input path/to/storyboard.json [--contract path/to/storyboard_contract_v1.1.0.json] [--junit path/to/report.xml]"
    );
    process.exit(3);
  }
