...
async function testRealComponentExtraction() {
  ...
  const testUrl =
    'https://images-cdn.zmangames.com/us-east-1/filer_public/25/32/253242c0-be63-4229-ad1e-7dd20b5fda83/zm7101_love_letter_rules.pdf';
  ...
  try {
    ...
    if (components && components.length > 0) {
      ...
      components.slice(0, 10).forEach((comp, i) => {
        console.log(
          `   ${i + 1}. ${comp.name || 'Unknown'} ${comp.quantity ? `(${comp.quantity})` : ''}`,
        );
      });
