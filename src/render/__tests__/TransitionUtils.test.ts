  ...
  describe('buildDebugGrid', () => {
    it('should generate a debug grid with default cell size', () => {
      ...
      expect(result.graph).toBe(
        '[v0]drawgrid=width=120:height=120:color=white@0.25:thickness=1,showinfo[grid0]',
      );
    ...
    it('should generate a debug grid with custom cell size', () => {
      ...
      expect(result.graph).toBe(
        '[v0]drawgrid=width=60:height=60:color=white@0.25:thickness=1,showinfo[grid0]',
      );
      expect(result.outV).toBe('grid0');
    });
  });
