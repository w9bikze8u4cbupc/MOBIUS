...
describe('validateUrl', () => {
  test('accepts valid HTTPS URLs', () => {
    const result = validateUrl('https://boardgamegeek.com/boardgame/155987/abyss', { allowHttpsOnly: true });
    expect(result.valid).toBe(true);
  });
  
  test('rejects HTTP URLs when HTTPS only is required', () => {
    const result = validateUrl('http://boardgamegeek.com/boardgame/155987/abyss', { allowHttpsOnly: true });
