...
export function extractComponentsFromText(pdfText, verbose = false) {
  ...
  function parseQuantityToken(q, note) {
    ...
    if (note) {
      ...
      if (noteText.includes('supply') || noteText.includes('unlimited') || 
          noteText.includes('bank') || noteText.includes('reserve') || 
  ...
  const CAPTION_OR_REWARD_PATTERNS = [
    ...
    { pattern: /\b(icon|image|illustration|diagram|figure|caption|labeled)\b/i, reason: 'image reference', reasonCode: 'image_reference' },
    { pattern: /\bexample\b/i, reason: 'example', reasonCode: 'example' },
    { pattern: /\bon the \d+(st|nd|rd|th)\s+space\b/i, reason: 'reward text', reasonCode: 'reward_text' },
    { pattern: /\b(win|receive|gain|draw|slide|reveal|place|move|advance|earn|collect|pay|take|flip|discard|shuffle)\b/i, reason: 'instruction', reasonCode: 'instruction' },
  ];
  ...
  if (start) {
    ...
    if (distinctLabels < 2 && formattedLines < 2) {
      if (verbose) {
        console.log('⚠️ LOW-CONFIDENCE FALLBACK SUPPRESSED: Not enough distinct labels or formatted lines');
      }
  ...
  for (const line of lines) {
    ...
    if (excluded) {
      if (verbose) {
        console.log(`   ❌ DROPPED: "${line}" (reason: ${exclusionReason}) [code: ${exclusionReasonCode}]`);
      }
      ...
      if (isSuspicious) {
        ...
        if (verbose) {
          console.log(`   📝 DEAD LETTER: "${line}" (suspicious but excluded - ${exclusionReason}) [code: ${exclusionReasonCode}]`);
        }
      }
    ...
    const mA = normalizedLine.match(/^\s*(\d+)\s+([A-Za-z][A-Za-z\s-]*?(?:cards?|tokens?|tiles?|board|boards|pearls?|cups?|keys?|lords?|locations?|monsters?))\s*:?(\s*\(([^)]*)\))?/i);
    if (mA) {
      if (verbose) {
        console.log(`   ✅ KEPT: "${line}" (reason: matched pattern with quantity) [code: matched_quantity]`);
      }
    ...
    const mB = normalizedLine.match(/place\s+the\s+(?:(\d+)\s+)?([A-Za-z][A-Za-z\s-]*?(?:tokens?|tiles?|board|boards|pearls?|cups?|keys?|lords?|locations?|monsters?))\b/i);
    if (mB) {
      if (verbose) {
        console.log(`   ❌ DROPPED: "${line}" (reason: setup instruction pattern) [code: setup_instruction]`);
      }
      continue;
    }
    ...
    if (mC) {
      ...
      if (canonicalName && ALLOWED_CANONICAL.has(canonicalName)) {
        if (verbose) {
          console.log(`   ✅ KEPT: "${line}" (reason: matched bare item with colon) [code: bare_item_colon]`);
        }
        addItem(mC[2], mC[1], null);
        continue;
      } else {
        if (verbose) {
          console.log(`   ❌ DROPPED: "${line}" (reason: not in allowlist) [code: not_in_allowlist]`);
        }
        continue;
      }
    }
    ...
    const mD = normalizedLine.match(/^\s*(one|two|three|four|five|six|seven|eight|nine|ten|eleven|twelve|thirteen|fourteen|fifteen|sixteen|seventeen|eighteen|nineteen|twenty|thirty|forty|fifty|sixty|seventy|eighty|ninety)\s+([A-Za-z][A-Za-z\s-]*?(?:cards?|tokens?|tiles?|board|boards|pearls?|cups?|keys?|lords?|locations?|monsters?))\s*:?(\s*\(([^)]*)\))?/i);
    if (mD) {
      if (verbose) {
        console.log(`   ✅ KEPT: "${line}" (reason: matched number word pattern) [code: number_word]`);
      }
    ...
    const mE = normalizedLine.match(/^([A-Za-z][A-Za-z\s-]*?(?:Pearls?|Cups?|Plastic\s+Cups?))\s*\(([^)]*)\)\s*$/);
    if (mE) {
      if (verbose) {
        console.log(`   ✅ KEPT: "${line}" (reason: matched item with parentheses) [code: item_with_parentheses]`);
      }
    ...
    const mG = normalizedLine.match(/^([A-Za-z][A-Za-z\s-]*?(?:pearls?|cups?|plastic\s+cups?))\s*\(([^)]*)\)\s*$/i);
    if (mG) {
      if (verbose) {
        console.log(`   ✅ KEPT: "${line}" (reason: matched item with parentheses) [code: item_with_parentheses]`);
      }
    ...
    if (breakdownMatch) {
      ...
      if (verbose) {
        console.log(`   📝 DEAD LETTER: "${line}" (reason: breakdown-only line) [code: breakdown_only]`);
      }
    } else if (verbose) {
  ...
  const order = ['Game board', 'Exploration cards', 'Lord cards', 'Location tiles', 'Monster tokens', 'Key tokens', 'Pearls', 'Plastic cups', 'Geisha cards', 'Item cards', 'Action markers', 'Victory markers'];
      ...
      if (item.note) {
        ...
        if (item.name === 'Exploration cards' && typeof item.count === 'number') {
          if (sum > 0 && sum !== item.count) {
            console.log(`   ⚠️  BREAKDOWN MISMATCH: ${item.name} total ${item.count} != sum of subtypes ${sum}`);
          }
        }
        ...
        if (item.name === 'Monster tokens' && typeof item.count === 'number') {
          ...
          if (sum > 0 && sum !== item.count) {
            console.log(`   ⚠️  BREAKDOWN MISMATCH: ${item.name} total ${item.count} != sum of multipliers ${sum}`);
          }
        }
  ...
  if (verbose) {
    ...
    if (deadLetterLines.length > 0) {
      ...
      deadLetterLines.forEach((item, i) => {
        console.log(`   ${i + 1}. "${item.line}" (reason: ${item.reason}) [code: ${item.reasonCode}]`);
      });
    }
  }
