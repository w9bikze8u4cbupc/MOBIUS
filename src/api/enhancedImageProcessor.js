...
export class EnhancedImageProcessor {
  ...
  async aiSegmentationRemoval(imagePath, config, tempFiles) {
    ...
    try {
      ...
      for (const bgColor of this.config.backgroundRemoval.backgroundColors) {
        ...
        const colorMask = await this.createColorMask(imagePath, bgColor, config.backgroundTolerance);
  ...
  async assessBackgroundRemovalQuality(processedImagePath) {
    try {
      ...
      const backgroundCleanliness = await this.assessBackgroundCleanliness(processedImagePath, stats) * 0.3;
      
      // Component preservation assessment (30% weight)
      const componentPreservation = await this.assessComponentPreservation(processedImagePath, metadata) * 0.3;
    ...
    try {
      ...
      if (metadata.hasAlpha) {
        ...
        for (let i = 3; i < data.length; i += 4) {
          if (data[i] < 128) { // Consider semi-transparent as background
            transparentPixels++;
          }
        }
        ...
        const avgVariance = (stats.channels || []).reduce((sum, channel) => 
          sum + (channel.stdev || 0), 0
    ...
    for (let attempt = 0; attempt < maxRetries; attempt++) {
      try {
        ...
        if (result.quality >= config.qualityThreshold) {
          console.log(`✅ Processing successful: ${result.method} (quality: ${(result.quality * 100).toFixed(1)}%)`);
          return {
            success: true,
          ...
          console.log(`⚠️ Quality below threshold: ${(result.quality * 100).toFixed(1)}% < ${(config.qualityThreshold * 100).toFixed(1)}%`);
        }
  ...
  async processComponentsParallel(componentList) {
    ...
    for (let i = 0; i < componentList.length; i += maxConcurrent) {
      ...
      console.log(`Processing batch ${Math.floor(i / maxConcurrent) + 1}/${Math.ceil(componentList.length / maxConcurrent)}`);
  ...
  shouldProcessComponent(component) {
    ...
    return component.area >= minArea && 
           (component.aspectRatio || 1) <= maxAspectRatio;
  }
