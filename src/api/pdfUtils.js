...
export async function extractTextFromPDF(pdfPath, requestId = null) {
  try {
    ...
    if (stats.size > 50 * 1024 * 1024) {
      // 50MB limit
      throw new Error(
        `PDF file too large (${(stats.size / (1024 * 1024)).toFixed(2)}MB). Maximum size is 50MB.`,
      );
    }

    LoggingService.info('PDFUtils', 'Starting PDF text extraction', { pdfPath, requestId });
    ...
    const firstPageText = data.text ? data.text.substring(0, 500) : '';
    LoggingService.info('PDFUtils', 'PDF text extraction completed', {
      pdfPath,
      requestId,
      textLength,
      firstPagePreview:
        firstPageText.replace(/\s+/g, ' ').substring(0, 200) +
        (firstPageText.length > 200 ? '...' : ''),
    });
    ...
    if (!data.text || data.text.trim().length === 0) {
      LoggingService.warn(
        'PDFUtils',
        'PDF text extraction yielded no text, attempting OCR fallback...',
        { pdfPath, requestId },
      );
      return await extractTextWithOCRFallback(pdfPath, requestId);
    }
    ...
    if (data.text.trim().length < 100) {
      LoggingService.warn(
        'PDFUtils',
        'PDF text extraction yielded minimal text, attempting OCR fallback...',
        { pdfPath, requestId },
      );
      return await extractTextWithOCRFallback(pdfPath, requestId);
    }
    ...
    LoggingService.error('PDFUtils', 'Error extracting text from PDF', {
      pdfPath,
      requestId,
      error: error.message,
    });
    // Try OCR fallback on error
    LoggingService.info('PDFUtils', 'Attempting OCR fallback...', { pdfPath, requestId });
    return await extractTextWithOCRFallback(pdfPath, requestId);
  }
}
...
export async function validatePDFFile(pdfPath) {
  try {
    console.log('DEBUG: validatePDFFile called with path:', pdfPath);
    ...
    try {
      ...
      const hasPdfSignature =
        fileBuffer && fileBuffer.length > 4 && fileBuffer.slice(0, 5).toString('ascii') === '%PDF-';
      if (!hasPdfSignature) {
        return {
          valid: false,
...
export async function extractImagesFromPDF(pdfPath, outputDir) {
  try {
    ...
    LoggingService.error('PDFUtils', 'PDF image extraction failed', {
      pdfPath,
      error: error.message,
    });
...
async function extractTextWithOCRFallback(pdfPath, requestId = null) {
  try {
    // Validate PDF file first
    const validation = await validatePDFFile(pdfPath);
    if (!validation.valid) {
      LoggingService.error('PDFUtils', 'PDF validation failed for OCR fallback', {
        pdfPath,
        requestId,
    ...
    LoggingService.info('PDFUtils', 'Starting OCR fallback process', { pdfPath, requestId });
    ...
    if (tesseractCheck.status !== 0) {
      LoggingService.warn(
        'PDFUtils',
        'Tesseract OCR not available, returning pdf_no_text_content error',
        { pdfPath, requestId },
      );

      // Return a special indicator that OCR is not available
    ...
    return new Promise((resolve) => {
      ...
      ocrProcess.on('close', (code) => {
        if (code === 0 && fs.existsSync(outputPath)) {
          try {
            ...
            LoggingService.info('PDFUtils', 'OCR fallback successful', { pdfPath, requestId });
            resolve(ocrText);
          } catch (readError) {
            LoggingService.error('PDFUtils', 'Failed to read OCR output', {
              pdfPath,
              requestId,
      ...
      setTimeout(() => {
        ocrProcess.kill();
        LoggingService.warn('PDFUtils', 'OCR process timed out', { pdfPath, requestId });
        resolve(trySmallerPDFApproach(pdfPath));
      }, 30000);
    });
  } catch (error) {
    LoggingService.error('PDFUtils', 'OCR fallback error', {
      pdfPath,
      requestId,
      error: error.message,
    });
...
async function trySmallerPDFApproach(pdfPath) {
  ...
  try {
    ...
    const message =
      'OCR processing failed. Please try:\n' +
    ...
    LoggingService.error('PDFUtils', 'Smaller PDF approach failed', {
      pdfPath,
      error: error.message,
    });
