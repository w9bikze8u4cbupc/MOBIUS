...
class BGGService extends BaseApiService {
  ...
  async searchGame(query) {
    try {
      ...
      if (!rateLimitResult.success) {
        ...
        throw new ApiError(this.serviceName, 'Rate limit exceeded for BGG API', 429);
      }

      const url = this.buildUrl(
        `${this.endpoints.search}?query=${encodeURIComponent(query)}&type=boardgame`,
      );
      ...
      throw error instanceof ApiError
        ? error
        : new ApiError(this.serviceName, 'Failed to search game', 500, error);
    }
  }
  ...
  async getGameDetails(gameId) {
    try {
      ...
      throw error instanceof ApiError
        ? error
        : new ApiError(this.serviceName, 'Failed to get game details', 500, error);
    }
  }
    ...
    try {
      ...
      if (response.status === 202 && attempt <= this.retryAttempts) {
        LoggingService.debug(
          this.serviceName,
          `Retrying request (${attempt}/${this.retryAttempts})`,
        );
