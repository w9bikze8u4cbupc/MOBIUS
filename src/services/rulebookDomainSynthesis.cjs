...
function buildDomainSynthesisPrompt(packet) {
  if(packet.preserveCandidateTitles)return buildDomainSynthesisPrompt({...packet,preserveCandidateTitles:false})
    + '\nOUTPUT IDENTITY REQUIREMENT: Return ONLY repairs of the supplied candidates. Copy each original candidate title VERBATIM so the deterministic reconciler can link the repair. Never add other rules. If unresolved, omit it. Allowed exact titles: '+JSON.stringify(packet.candidates.map(c=>c.title));
  if (packet.kind === 'document-completeness' && packet.mode !== 'validator-guided-corrective') {
    return buildDomainSynthesisPrompt({...packet,kind:null}) + '\n\nDOCUMENT COMPLETENESS PASS: Read EVERY supplied full-page excerpt, not only the first keyword match. Extract all essential procedures, choices, costs, limits, exceptions and dependencies needed to set up and play a first multiplayer game. Related steps may be grouped in one precise atom. Do not re-emit an existing claim unchanged. If an existing claim is incomplete, emit a separate complementary claim with the missing condition or procedure, never silently replace the original. Skip solo-only/expansion rules, lore, licensing text and decorative OCR. Existing claims below are structural comparison context ONLY, never source authority. Every new fact MUST cite supplied official evidence.\n' + JSON.stringify(packet.existingClaims || []);
  }
