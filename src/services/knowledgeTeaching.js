...
export const KNOWLEDGE_TEACHING_CONTRACT = 'mobius-source-grounded-teaching-localization-v1';
...
export async function materializeKnowledgeTeaching({model,language='fr-CA',cachePath,env=process.env,complete}={}) {
  ...
  const props={atomId:{type:'string'},heading:{type:'string'},narration:{type:'string'},displayLines:{type:'array',items:{type:'string'}},sourcePages:{type:'array',items:{type:'integer'}}};
...
export function applyKnowledgeTeaching(model, receipt) {
  ...
  return {...model,teachingProvenance:{contract:receipt.contract,inputHash:receipt.inputHash,provider:receipt.provenance},
    ruleAtoms:model.ruleAtoms.map(rule=>{const l=lessons.get(rule.id);return {...rule,teaching:{...rule.teaching,heading:l.heading,narration:l.narration,displayLines:l.displayLines,profile:'AMELIE_TEACHING_WARM_R10'}};})};
}
